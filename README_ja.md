...
| **ヘッドレス CLI エージェント**  | Gemini CLI、Claude Code、OpenAI Codex、OpenCode、DeepSeek Harness 向けの最小イメージ — デスクトップ不要、SSH のみで動作。         |
| **すぐに使える**                 | Antigravity IDE と Git が事前インストール済み。デスクトップ版にはブラウザ（amd64 は Google Chrome、arm64 は Chromium）も同梱。待ち時間ゼロで即座に開始。 |
...
| **検証付き IDE 更新**            | `ide update` は現在の Antigravity IDE ビルドを Google から取得し、Google が公開するチェックサムと照合してから置き換えます。 |
...
`ag` は Antigravity IDE を Google 公式の tarball からインストールし、バージョンと build id で固定した上で、Google がその成果物に対して公開している sha256 と照合します。自動更新は無効なので、サンドボックスは明示的に更新を要求するまでビルド時のビルドに留まります。IDE には自身の agent バックエンドが固定して使う証明書が同梱されており、2.5.5 のそれは 2026-11-03 に失効します。そのため失効まで 14 日を切るとイメージのビルドは意図的に失敗し、無言で停止した Agent Manager を出荷しません（[#42](https://github.com/Shiritai/sanity-gravity/issues/42)）。

- **ホスト側**: `sanity-cli` がコンテナのライフサイクル全体を管理。メンテナンスコマンドはまず**最新の `gravity-cli` をターゲットコンテナに自動注入**するので、古い checkout から作られたコンテナでも現在のインストーラが動きます。
- **コンテナ内部**: `gravity-cli` が Google の更新サービスに現在のビルドを問い合わせ、ダウンロードし、そのビルドに対して Google が公開する sha256 と照合してから入れ替えます。イメージのビルドが実行するのと同じスクリプトなので、サンドボックスの wrapper とデスクトップエントリの出所は一つです。

#### ホストから操作

```bash
# 現在の IDE ビルドをダウンロード・検証してインストール
./sanity-cli ide update --name sanity-gravity

# 同上に加えて、バージョン変更後も残る Electron キャッシュを削除
