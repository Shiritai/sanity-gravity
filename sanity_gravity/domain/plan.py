...
NEVER_CACHED: CacheProbe = lambda ref: False  # noqa: E731 - a named constant


@dataclass(frozen=True)
class BuildRequest:
    """Everything the planner needs, parsed into structure at the edge."""

    roots: tuple[LayerRef, ...]
    no_cache: bool = False
    #: Kinds rebuilt even when present locally. The final image only,
    #: matching the historical "final tag always rebuilds" rule.
    always_build: frozenset[LayerKind] = frozenset({LayerKind.CONNECTOR})
...
def plan(
    ...
    *,
    ...
    """The whole planner: one loop, one cache decision site."""
    nodes: list[PlanNode] = []
    for ref in closure(request.roots):
        if ref.kind not in request.always_build and probe(ref):
            on_cache_hit(ref)
            continue
