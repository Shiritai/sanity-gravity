...
string is :meth:`sanity_gravity.domain.naming.Naming.layer`; the only
place a string becomes a LayerRef is ``Naming.parse_layer``. Everything
in between - planning, parent resolution, ``--layer`` filtering, cache
probing - moves values.
"""
...
class LayerRef:
    ...
    def tag(self) -> Tag | None:
        if self.kind is not LayerKind.CONNECTOR:
            return None
        # Field occupancy is guaranteed by __post_init__ (_FIELDS table).
        assert self.agent is not None and self.desktop is not None
        assert self.connector is not None
        return Tag(agent=self.agent, desktop=self.desktop, connector=self.connector)
