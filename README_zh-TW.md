...
每個映像檔由一個標籤描述：**`{base-image-}agent-desktop-connector`**。根據你的用途選擇。基礎映像可省略，預設為 Ubuntu；其他基礎映像以字首標記（例如 `debian-ag-xfce-kasm`）：
...
共有 **46 個有效組合**（預設 Ubuntu 基礎映像，加上每個標籤的 `debian-` 前綴變體）。完整矩陣、維度模型與約束規則請參考 [模組化標籤系統](docs/tags.md)。
