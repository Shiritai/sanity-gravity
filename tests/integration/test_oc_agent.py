...
OC_SSH_IMAGE = "sanity-gravity:oc-none-ssh"
...
pytestmark = pytest.mark.skipif(
    not _image_exists(OC_SSH_IMAGE),
    reason=f"{OC_SSH_IMAGE} not built; run ./sanity-cli build oc-none-ssh",
)
...
class TestOpenCodeCLIAgent:
    ...
    def test_oc_startup(self, clean_container, docker_cli, host_env, free_port):
    ...
    def test_oc_binary_exists(self, clean_container, docker_cli, host_env):
        """OpenCode ships as a standalone Bun binary (no Node.js required)."""
        container_name = clean_container("sanity-test-oc-binary")
        docker_cli.run_container(name=container_name, image=OC_SSH_IMAGE, env=host_env)
        time.sleep(2)

        result = docker_cli.exec(container_name, "which opencode")
        assert "/opencode" in result.stdout.strip()

    def test_oc_opencode_installed(self, clean_container, docker_cli, host_env):
    ...
    def test_oc_user_mapping(self, clean_container, docker_cli, host_env):
    ...
    def test_oc_opencode_accessible_as_user(self, clean_container, docker_cli, host_env):
    ...
    def test_oc_headless_no_display(self, clean_container, docker_cli, host_env):
