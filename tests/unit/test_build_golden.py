...
printing) is covered by the stacked series' verb suites; this file pins
the kernel downward.
"""
...
_OFFICIAL = [resolve_tag(t) for t in OFFICIAL_TAGS]


def _official_desktops() -> list[str]:
    return sorted({t.desktop for t in _OFFICIAL})


def _official_pairs() -> list[tuple[str, str]]:
    return sorted({(t.agent, t.desktop) for t in _OFFICIAL})
...
class _Events:
    """Reporter sink recording every event (the cache-hit channel)."""
...
class _Capture:
    """Executor stand-in recording enqueued actions verbatim."""
