...
from sanity_gravity.cli.registry import OFFICIAL_TAGS, parse_tag
...
def _official_desktops() -> list[str]:
    return sorted({parse_tag(t)[1] for t in OFFICIAL_TAGS})


def _official_pairs() -> list[tuple[str, str]]:
    return sorted({(parse_tag(t)[0], parse_tag(t)[1]) for t in OFFICIAL_TAGS})
