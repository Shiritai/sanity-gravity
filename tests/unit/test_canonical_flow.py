...
def test_up_identities_agree_across_modules(monkeypatch, tmp_path):
    ...
    def fake_try_run(cmd, **kwargs):
        ...
        # auto-pull.
        out = '[{"Id": "x"}]' if "inspect" in _cmd_text(cmd) else ""
        return Completed(tuple(cmd), 0, stdout=out)

    def fake_capture(cmd, **kwargs):
        calls.append(cmd)
        # The collision probe answers "nothing found".
        return ""
    ...
    monkeypatch.setattr(up_mod, "try_run", fake_try_run)
    monkeypatch.setattr(up_mod, "capture", fake_capture)
    monkeypatch.setattr(
...
def test_pull_refs_agree_with_naming(monkeypatch):
    ...
    def fake_try_run(cmd, **kw):
        calls.append(tuple(cmd))
        return Completed(tuple(cmd), 0)

    monkeypatch.setattr(pull_mod, "try_run", fake_try_run)
