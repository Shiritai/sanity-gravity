...
# test now forces this set to track reality.)
_ALLOWED = {
...
# test_prefix_constants_only_in_naming. Derived, not restated: the
# mirror test above _PREFIX_VALUES already pins the set to reality, so
# a second hand-kept list would only be a second place to rot (the
# LAYER_PREFIX ghost lived here).
_PREFIX_NAMES = frozenset(_PREFIX_VALUES)
...
def test_prefix_constants_mirror_naming_reality():
    """Stale check for the guard's own restated-literal list. The list
    must stay literal (importing Naming would weld the guard to the
    module it guards), so the cross-check reads naming.py as AST text:
    ...
    declared = {
        ...
        for node in naming_cls.body
        if isinstance(node, ast.AnnAssign)
        and isinstance(node.target, ast.Name)
        and isinstance(node.value, ast.Constant)
        and isinstance(node.value.value, str)
    }
    assert declared == _PREFIX_VALUES, (
        "the guard's _PREFIX_VALUES and Naming's string constants "
        f"disagree - ghosts or renames: guard-only "
        f"{sorted(set(_PREFIX_VALUES) - set(declared))}, naming-only "
        f"{sorted(set(declared) - set(_PREFIX_VALUES))}"
    )


def test_allowed_exemptions_are_load_bearing():
    """A file exempt from the identity-f-string scan must actually
    render at least one identity family; otherwise the exemption is a
